var $PhoneGapDB = {
	...
	//			tx.executeSql("DROP TABLE IF EXISTS USER");
				tx.executeSql("CREATE TABLE IF NOT EXISTS USER (playerName varchar ,  date Date)");
				//
			}
			function successDB() 
			{
			...
			function queryDB(tx) 
			{
				tx.executeSql('SELECT * FROM USER', [], renderEntry, errorDB);
			}
			function renderEntry(tx,results)
			{
				if(results.rows.length==0)
				{
					$PhoneGapDB.playerName  =  'noName';
					

				}
				else
				{
					$PhoneGapDB.playerName  =  results.rows.item(0).playerName;
				}
			}
       ...
       ,
       insertUser: function(playerName,date){
    	   ...
    	   var db = window.openDatabase("BrainTrainer", "1.0", "BrainTrainer", 100000);
			...
			function queryDB(tx) 
			{
				tx.executeSql("SELECT * FROM USER where playerName='"+playerName+"'", [], renderEntry, errorDB);
			}
			function renderEntry(tx,results)
			{
				...
				{
					var date = new Date();
					tx.executeSql("INSERT INTO USER (playerName, date) VALUES ('"+playerName+"','"+date+"')");
				}
				else
				{
					playerName  =  results.rows.item(0).playerName;
				}
			}
