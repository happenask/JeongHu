		...
		<table border="0"  width="800" height="300" cellspacing="30" class ="productTable">
			...
			<div style="text-decoration: none;text-align: left;padding-left: 50px;">
				<c:choose>
					<c:when test= "${requestScope.pto.previousPageGroup}">
						<a href ="/kostaWebS/model/cate_list.do?page=${requestScope.pto.startPageOfPageGroup-1 }&cateNumber=${num}" class="link">[이전]</a>
					</c:when>
				</c:choose>
			<c:forEach var="i" begin="${requestScope.pto.startPageOfPageGroup}"  end="${requestScope.pto.endPageOfPageGroup}" step="1">
				<c:choose>
					...
					<c:otherwise>
							<a href = "/kostaWebS/model/cate_list.do?page=${i}&cateNumber=${num}" class="link"><font color="#191919">[${i}]</font></a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
				<c:if test= "${requestScope.pto.nextPageGroup}">
				<a href ="/kostaWebS/model/cate_list.do?page=${requestScope.pto.endPageOfPageGroup+1 }&cateNumber=${num}" class="link">[다음]</a>
				</c:if>
			</div>
