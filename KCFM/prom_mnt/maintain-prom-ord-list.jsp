<%
/** ############################################################### */
/** Program ID   : maintain-prom-ord-list.jsp                       */
/** Program Name : maintain-prom-ord-list       					*/
/** Program Desc : 관리자-홍보물 주문내역							*/
...
<%
	...
	log4u.log("CALL /maintain-prom-ord-list.jsp");
...
<%--     	$frm.action = "<%=root%>/prom_mnt/maintain-prom-ord-list.jsp"; --%>
    	$frm.attr("action",   "<%=root%>/prom_mnt/maintain-prom-ord-list.jsp");
    ...
    function fnShowListDetail(groupCd,corpCd,brandCd,storeCd,ordNo, insertUserId){
    	...
    	$frm.attr("action",   "<%=root%>/prom_mnt/prom_ord-dtl-view.jsp");
    ...
    function comboCorpCd(){
    	...
    	$("input[name=hGroupCd]").attr("value","<%=hGroupCd%>"); //기업코드
		...
		$.ajax(
			{
				url      : "<%=root%>/prom_mnt/maintain-prom-ord-list-combo.jsp",
				type     : "POST",
...
// 	}
	...
	function fnSavePopUpInfo(){
		...
		$frm.attr("action",   "<%=root%>/prom_mnt/prom_ord-dtl-view_write_ok.jsp" );
		$frm.attr("target",   "iWorker");
        $frm.attr("method",   "post");
	...
	function fnSavePopUpResult(msg){
		...
		$("input[name=hInsertUserId]").attr("value", "");
	...
	function fnSelectComment(){
		...
		$.ajax(
			{
				url      : "<%=root%>/prom_mnt/prom_ord-dtl-view_comment.jsp?gubun=select",
				type     : "POST",
				data     : $("#formdata").serialize(),
				dataType : "html", 
				success  : function(data)
	...
	function fnWriteComment(num){
		...
		$.ajax(
			{
				url      : "<%=root%>/prom_mnt/prom_ord-dtl-view_comment.jsp?gubun=insert",
				type     : "POST",
				data     : $("#formdata").serialize(),
				dataType : "html", 
				success  : function(data)
 	...
 	<div id="wrap" >
	 	<section class="header-bg">
 		  ...
 		  <section class="contents admin">
 		 	<header>
 		 		<h1>◎ <span>홍보물 관리 &gt; 주문내역 확인</span></h1>
...
<!-- 				        			<th width="0">매장코드</th> -->
		...
		%>
			    			<tr>
				      				...
				      				<td width="100" class="txt-cnt"><%=orderBean.get홍보물명()%></td> 
