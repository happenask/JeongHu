...
<%
/** ############################################################### */
/** Program ID   : maintain-prom-dtl.jsp                            */
/** Program Name :  maintain-prom-detail	       					*/
...
<%
	...
	//-------------------------------------------------------------------------------------------------------
	if ("".equals(inCompanyCd) || inCompanyCd == null || sseCustAuth.equals("90"))
	{
...
<head>
    ...
    function chgCorp(val){
		...
		$.ajax(
			{
				url      : "<%=root%>/prom_mnt/maintain-prom-dtl-combo.jsp",
				type     : "POST",
    ...
    function fnShowListDetail(group, corp, brand, code, no){
    	...
    	frm.action = "<%=root%>/prom_mnt/prom-dtl-view.jsp";
    	frm.target = "iWorker";
    	frm.submit();
	...
	{ 	
		...
		frm.inEvent.value = "new";
 		frm.action = "<%=root%>/prom_mnt/maintain-prom-regist.jsp";
 		frm.target = "_self";
		frm.submit();
	...
	function fnDeleteWriting(group, corp, brand,menu ,code, no)
	{
		...
		{	
		    ...
		    frm.inEvent.value     = "del";
	 		frm.action = "<%=root%>/prom_mnt/prom-delete-ok.jsp";
	 		frm.target = "_self";
			frm.submit();
		}
    }	
 	...
 	<div id="wrap" >
	 	...
	 	<section id="info-panel">
 		  ...
 		  <section class="contents admin">
 		 	<header>
 		 		<h1>◎ <span>홍보물 관리 &gt; 홍보물 상세 관리</span></h1>
 		 	
	        	<div class="search-d">
		    	<!-- 	<label for="sDate" > ▶ 등록 기간 : </label>
