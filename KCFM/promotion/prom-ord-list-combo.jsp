<%
/** ###############################################################*/
/** Program ID   : /promotion/prom-ord-list-combo.jsp              */
/** Program Name : 프로모션 > 법인코드로 브랜드코드, 매장코드 조회       */
/** Program Desc : 프로모션 > SELECT BOX 정보 조회                      */
...
// 	System.out.println("---------------------[프로모션 > SELECT BOX 조회]--------------");
// 	System.out.println("권한코드   >>>>>>>>>>>>>>>>>>>> " + sseCustAuth);
// 	System.out.println("조회구분   >>>>>>>>>>>>>>>>>>>> " + hComboGubun);
	...
	if("corpCombo".equals(hComboGubun)){ //법인조회
		...
		if( list != null && list.size() > 0 ) 
		{
			...
			for( int i = 0; i < list.size(); i++ ) 
			{
				...
				if("41".equals(sseCustAuth) || "90".equals(sseCustAuth)){ //전단지 업체[41], 슈퍼유저[90]
